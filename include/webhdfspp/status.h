 ...
 */
#ifndef WEBHDFSPP_STATUS_H_
#define WEBHDFSPP_STATUS_H_
...
public:
  ...
  static Status Error(const char *msg) { return Status(kGenericError, msg); }
  static Status InvalidEncryptionKey(const char *msg) {
    return Status(kInvalidEncryptionKey, msg);
  }
  static Status Exception(const char *expception_class_name,
  ...
  int code() const {
    return (state_ == NULL) ? kOk : static_cast<int>(state_[0]);
  }
...
private:
  ...
  //    state_[0]    == code
  //    state_[4]    == the length of the first message
  //    state_[8]    == the length of the second message
  const char *state_;
  ...
  static const char *ConstructState(int code, const char *msg1,
                                    const char *msg2);
};
